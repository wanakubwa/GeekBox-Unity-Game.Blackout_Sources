...
public class GameManager : ManagerSingletonBase<GameManager>
{
    ...
    public void LoadGameScene(string scenarioDirectoryName, bool isSavedScenario = false)
    {
        ScenariosManager scenariosManager = ScenariosManager.Instance;
        if(scenariosManager != null)
        {
            scenariosManager.LoadOfficialScenario(Managers, scenarioDirectoryName);
        }
    }
    ...
    private void LoadSceneByActualLabel()
    {
        switch(ActualSetScene)
        {
            case SceneLabel.GAME:
                PopUpManager.Instance.ShowScenarioSelectPopUp();
                break;
            case SceneLabel.MAIN_MENU:
                InitializeMenuScene();
                break;
            default:
                break;
